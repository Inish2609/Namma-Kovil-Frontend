...
import { createEvent, getEventBookedSlotByDate } from '../services/service';
import { convertTo24Hour, formatDateTime, validateField } from '../utils/helper';
...
export default function PoojaRegistrationScreen() {
  ...
  useEffect(() => {
    if (selectedDate) {
      handleGetBookedSlots(selectedDate);
    }
  }, [selectedDate, reset]);
  ...
  async function handleGetBookedSlots(date: Date) {
    try {
      const formattedDate = `${date.getFullYear()}-${(date.getMonth() + 1)
        .toString()
        .padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}`;

      const response = await getEventBookedSlotByDate(formattedDate);

      setBookedSlots(response.data.value[0]?.booked_slots ?? []);
    } catch (err) {
      showAlert('Error', 'Fetching Booking Slot Failed!!');
      console.log(err);
    }
  }
  ...
  async function handleCreateEvent() {
    try {
      ...
      const poojaNameValidator = validateField(selectedPooja, 'required')
      if(poojaNameValidator) {
        showAlert("Error", poojaNameValidator)
        return
      }
      const dateValidator = validateField(selectedDate, 'required')
      if(dateValidator) {
        showAlert("Error", dateValidator)
        return
      }
      const slotValidator = validateField(selectedSlot, 'required')
      if(slotValidator) {
        showAlert("Error", slotValidator)
        return
      }
      const devoteeNameValidator = validateField(devoteeName, 'text')
      if(devoteeName) {
        showAlert("Error", devoteeNameValidator)
        return
      }
      const devoteePhoneNumberValidator = validateField(devoteePhoneNumber, 'phone')
      if(devoteePhoneNumberValidator) {
        showAlert("Error", devoteePhoneNumberValidator)
        return
      }
      const devoteeAddressValidator = validateField(devoteeAddress, 'text')
      if(devoteeAddressValidator){
        showAlert("Error", devoteeAddressValidator)
        return
      }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroContainer}>
          ...
          <View style={styles.heroContent}>
            <Text style={styles.heroTitle}>🛕 Sacred Pooja Booking</Text>

            <Text style={styles.heroSubTitle}>Receive Divine Blessings</Text>
          </View>
        </View>

        {/* Select Pooja */}

        <Text style={styles.sectionTitle}>Select Pooja</Text>
        ...
        >
          {poojas.map(item => (
            <TouchableOpacity
              ...
              style={[
                styles.poojaCard,
                selectedPooja === item.id && styles.selectedPoojaCard,
              ]}
            >
              ...
              <Image source={item.image} style={styles.poojaImage} />

              <Text
                style={[
                  styles.poojaName,
                  selectedPooja === item.id && styles.selectedText,
                ]}
              >
                {item.name}
              </Text>
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  isBooked && styles.bookedSlotCard,
                ]}
              >
                <Text
                  style={[
        ...
        <View style={styles.summaryCard}>
          ...
          >
            <Text style={styles.registerText}>Register Pooja</Text>
          </TouchableOpacity>
        </View>
